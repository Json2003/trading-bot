...
def send_slack_block(job_alert: dict):
    ...
    model_version = (
        result.get("summary", {}).get("model_version")
    ...
    blocks = [
        {
            "type": "section",
            "text": {"type": "mrkdwn", "text": f"*Job {job_id}* - Status: *{status.upper()}*"},
        },
    ]
    if model_version:
        blocks.append(
            {
                "type": "section",
                "text": {"type": "mrkdwn", "text": f"Model version: `{model_version}`"},
            }
        )
    if duration:
        blocks.append(
            {
                "type": "context",
                "elements": [{"type": "mrkdwn", "text": f"Duration: {duration:.1f}s"}],
            }
        )
    if log_path:
