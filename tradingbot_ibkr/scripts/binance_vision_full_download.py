...
def main():
    ...
    ap.add_argument(
        "--merge",
        choices=("csv", "parquet"),
        help="merge downloaded CSVs into one output file (csv or parquet)",
    )
    ap.add_argument(
        "--merge-only",
        action="store_true",
        help="skip downloading and only merge existing CSVs in the out dir",
    )
    ap.add_argument("--verify", action="store_true", help="verify CSV consistency before merging")
    ap.add_argument(
        "--merged-name",
        default=None,
        help="filename (without ext) for merged output; defaults to {symbol}_trades",
    )
    args = ap.parse_args()
    ...
    threads = [
        threading.Thread(
            target=worker, args=(q, pbar, args.remove_zip, not args.no_unzip), daemon=True
        )
