...
def run_pretrain(
    ...
    trainer.model = preprocessing.StandardScaler() | ensemble.BaggingClassifier(
    ...
    for epoch in range(epochs):
        for i, path in enumerate(paths):
            ...
            examples = build_examples_from_df(
                df, max_samples=max_samples, feature_cols=feature_cols
            )
            labels = label_examples_with_horizon(
                df, examples, horizon=horizon, profit_pct=profit_pct
            )
            n = len(examples)
            ...
            if job_file:
                _write_job_update(
                    ...
                    status="running",
                    progress=0,
                    message=f"epoch {epoch + 1} file {i + 1}/{len(paths)}: {n} examples",
                )
    ...
    try:
        ...
        if job_file:
            _write_job_update(
                job_file, status="error", progress=100, message=f"model save failed: {e}"
            )
...
def main():
    ...
    parser.add_argument(
        "--files",
        nargs="+",
        default=[str(p / "datafiles" / "*.csv")],
        help="CSV files or glob(s) to load",
    )
    parser.add_argument("--epochs", type=int, default=1)
    parser.add_argument("--max-samples", type=int, default=None)
    parser.add_argument(
        "--threshold", type=float, default=0.5, help="reporting threshold for online predictions"
    )
    parser.add_argument(
        "--features",
        type=str,
        default=None,
        help="comma-separated feature columns to use (default basic set)",
    )
    parser.add_argument(
        "--horizon", type=int, default=12, help="label horizon in bars to look ahead"
    )
    parser.add_argument(
        "--profit-pct", type=float, default=0.01, help="profit pct used to define positive label"
    )
