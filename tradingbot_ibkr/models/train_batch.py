...
def create_features(
    df: pd.DataFrame, lookback_periods: List[int] = [5, 10, 20]
...
>>>>>>> origin/main
    ...
    def train_all_models(
        ...
        if valid_results:
            best_model_name = max(
                valid_results.keys(), key=lambda k: valid_results[k]["cv_mean_score"]
            )
            ...
            for model_name, result in results.items():
                if "error" not in result:
                    ...
                    logger.info(
                        f"{result['model_name']}: {score:.4f} (+/- {std * 2:.4f}) - {time_taken:.1f}s{marker}"
                    )
                else:
            ...
            best_model_summary = {
                ...
                "all_results": {
                    k: {kk: vv for kk, vv in v.items() if kk != "model_object"}
...
def train_and_evaluate_models(
    ...
    try:
        ...
        # Initialize trainer
        trainer = OptimizedModelTrainer(use_optuna=use_optuna, n_trials=n_trials, cv_folds=5)
