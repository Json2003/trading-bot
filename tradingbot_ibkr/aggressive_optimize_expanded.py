...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("optimization_expanded.log"), logging.StreamHandler()],
)
...
def backtest_combination(args):
    ...
    try:
        ...
        if combo_idx % 20 == 0 or combo_idx <= 10:  # Log first 10 and every 20th
            logger.info(
                f"[{combo_idx}/{total_combos}] tp={tp:.3f} sl={sl:.3f} hold={hold} risk={risk:.3f} | WR={result['win_rate']:.1f}% PnL={result['pnl']:.1f} Trades={result['trades']} ({execution_time:.2f}s)"
            )
        ...
        logger.error(
            f"[{combo_idx}/{total_combos}] Failed: tp={tp} sl={sl} hold={hold} risk={risk} - {str(e)}"
        )
        return None
...
def save_progress_checkpoint(results, output_path, completed, total, start_time, metadata=None):
    ...
    checkpoint_data["results"] = sorted(
        results, key=lambda x: (x["win_rate"], x["pnl"]), reverse=True
    )
    ...
    logger.info(
        f"Checkpoint saved: {completed}/{total} ({completed / total * 100:.1f}%) - Best WR: {checkpoint_data['metadata']['statistics']['best_win_rate']:.2f}%"
    )
...
def run_grid(symbol="BTC/USDT", max_workers=None, batch_size=50):
    ...
    logger.info(
        f"Using {max_workers or 'auto-detected'} parallel workers, batch size: {batch_size}"
    )
    ...
    combo_args = [
        (df, tp, sl, hold, risk, idx, total_combos)
        ...
        for future in as_completed(future_to_combo):
            ...
            if completed_count % batch_size == 0:
                save_progress_checkpoint(
                    results, output_path, completed_count, total_combos, start_time
                )
    ...
    if results_sorted:
        ...
        logger.info(
            f"Best result: WR={best['win_rate']:.2f}% PnL={best['pnl']:.2f} Trades={best['trades']}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Expanded grid search optimization with parallel processing"
    )
    parser.add_argument(
        "--symbol", default="BTC/USDT", help="Trading symbol to optimize (default: BTC/USDT)"
    )
    parser.add_argument(
        "--workers", type=int, help="Number of parallel workers (default: auto-detect)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Checkpoint save interval (default: 50)"
    )

    args = parser.parse_args()

    try:
        run_grid(symbol=args.symbol, max_workers=args.workers, batch_size=args.batch_size)
