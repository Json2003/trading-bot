...
>>>>>>> origin/main
    ...
    status = {
        ...
        "metrics": {
            ...
            "pnl": stats.get("pnl"),
            "overall_max_drawdown_pct": stats.get("equity_curve")
            and max(
                (
                    max(e["balance"] for e in stats["equity_curve"])
                    - min(e["balance"] for e in stats["equity_curve"])
                )
                / max(e["balance"] for e in stats["equity_curve"])
                * 100.0,
                0,
...
def main():
    ...
    p.add_argument(
        ...
        type=float,
        default=0.005,
        help="Fraction of capital to allocate to canary (e.g., 0.005 = 0.5%)",
    )
