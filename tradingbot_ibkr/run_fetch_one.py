...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("data_fetch.log"), logging.StreamHandler()],
)
...
class FREDDataFetcher:
    ...
    def _make_request(self, endpoint: str, params: Dict) -> Optional[Dict]:
        ...
        request_params = {"api_key": self.api_key, "file_type": "json", **params}
    ...
    def _clean_and_validate_data(self, df: pd.DataFrame, series_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logger.info(
                f"Date range: {df['date'].min()} to {df['date'].max()} ({date_range.days} days)"
            )

            # Validate value range
            value_stats = df["value"].describe()
            logger.info(
                f"Value statistics: min={value_stats['min']:.3f}, max={value_stats['max']:.3f}, mean={value_stats['mean']:.3f}"
            )
