...
def compute_trade_metrics(df: pd.DataFrame):
    ...
    if "pnl" in df.columns:
        df["outcome"] = np.where(
            df["pnl"] > 0, "win", np.where(df["pnl"] < 0, "loss", "break_even")
        )
    else:
        df["outcome"] = "unknown"
...
def main():
    ...
    p.add_argument(
        "--out",
        default="tradingbot_ibkr/datafiles/self_eval_report.json",
    ...
    if df is None:
        print(
            "No trade log found. Please write trades to tradingbot_ibkr/datafiles/trades.csv or trades.jsonl with fields: entry_ts, exit_ts, entry_price, exit_price, size, pnl, model_version, entry_reason, exit_reason"
        )
        return
                ...
                alerts.append(
                    {
                        ...
                        "prev": prev["win_rate"],
                        "now": report["win_rate"],
                        "msg": "win_rate dropped >5pp",
                    }
                )
        # check avg_pnl drop > 20%
        if (
            prev.get("avg_pnl") is not None
            and report.get("avg_pnl") is not None
            and prev["avg_pnl"] != 0
        ):
