...
def calculate_performance_metrics(
    equity_curve: List[float], trades: List[Dict], risk_free_rate: float = 0.02
) -> Dict:
    """
...
>>>>>>> origin/main
    ...
    logger.info(
        f"Performance metrics calculated: Sharpe={sharpe_ratio:.3f}, Max DD={max_drawdown * 100:.2f}%, Win Rate={win_rate * 100:.1f}%"
    )
...
>>>>>>> origin/main
    ...
    if enable_logging:
        ...
        logger.info(
            f"  Parameters: TP={take_profit_pct * 100:.2f}%, SL={stop_loss_pct * 100:.2f}%, Hold={max_holding_bars}"
        )
...
>>>>>>> origin/main
            ...
            # Trailing stop
                ...
                and row["low"] <= trailing_stop_price
            ):
            ...
            if exit_triggered:
                # Record exit
                trades.append(
                    {"type": exit_type, "time": timestamp, "price": exit_price, "qty": qty}
                )
                ...
                if slippage_vs_volume and "volume" in row:
                    try:
                        recent_vol = (
                            float(row["vol_mean20"]) if not pd.isna(row["vol_mean20"]) else 1.0
                        )
                        extra_slippage = min(
                            slippage_k * (qty / max(recent_vol, 1e-8)), slippage_cap
                        )
