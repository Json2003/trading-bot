...
>>>>>>> origin/main
        ...
        logger.debug(
            f"[{combo_idx}/{total_combos}] Completed - WR: {result['win_rate']:.2f}%, PnL: {result['pnl']:.2f}"
        )
        return result
...
def should_prune_parameter(results, param_name, param_value, threshold_percentile=25):
    ...
    if should_prune:
        logger.info(
            f"Pruning parameter {param_name}={param_value} (median WR: {param_median:.2f}% < threshold: {overall_threshold:.2f}%)"
        )
...
def save_intermediate_results(results, output_path, combo_idx, total_combos):
    ...
    logger.info(
        f"Intermediate results saved ({combo_idx}/{total_combos} - {combo_idx / total_combos * 100:.1f}%)"
    )
...
>>>>>>> origin/main
    ...
    logger.info(
        f"Processing {len(combo_args)} parameter combinations with {max_workers or 'auto'} workers"
    )
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            if enable_pruning and len(results) >= 20 and len(results) % 10 == 0:
                for param_name in ["tp", "sl", "hold", "risk"]:
                    param_values = list(
                        set(
                            params[
                                f"{param_name}_pct" if param_name != "hold" else "max_holding_bars"
                            ][0]
                            for r in results
                        )
                    )
            ...
            if no_improvement_count >= early_stopping_patience:
                logger.info(
                    f"Early stopping triggered after {no_improvement_count} combinations without improvement"
                )
                break
