        ...
        data.append(
            {
                "ts": dates[i]
                if i < len(dates)
                else dates[-1] + timedelta(hours=i - len(dates) + 1),
                "open": open_price,
                "high": high,
                "low": low,
                "close": close_price,
...
<<<<<<< HEAD
    ...
    try:
        ...
        results = aggressive_strategy_backtest(
            df, take_profit_pct=0.02, stop_loss_pct=0.01, max_holding_bars=24, enable_logging=True
        )
...
<<<<<<< HEAD
    ...
    try:
        ...
        for model_name in successful_models:
            ...
            logger.info(
                f"  {result.get('model_name', model_name)}: CV Score = {result.get('cv_mean_score', 0):.4f}"
            )
...
def run_comprehensive_test():
    ...
    for test_name, test_func in tests:
        try:
            logger.info(f"\n🧪 Testing {test_name}...")
            # individual test functions use assertions; if no exception, it's a pass
            test_func()
    ...
    if passed_tests == total_tests:
        ...
        raise AssertionError(f"{total_tests - passed_tests} tests failed")
