...
app = FastAPI(title="Trading Bot Live Server", version="1.1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
STATE: Dict[str, Any] = {
    ...
    "ohlcv": {},  # symbol -> list of candles {ts, o,h,l,c,v}
    "server_stats": {
    ...
    "perf": {
        "loop_durations_ms": deque(maxlen=200),
        "trades_refresh_ts": None,
    },
}
...
def refresh_trades_from_file(max_recent: int = 20) -> None:
    ...
    try:
        ...
        # Track equities over time for sparkline/drawdown
        STATE["perf"]["trades_refresh_ts"] = time.time()
        STATE.setdefault("equity_history", [])
        STATE["equity_history"].append({"ts": datetime.now(timezone.utc).isoformat(), "equity": STATE["metrics"]["equity"]})
        STATE["equity_history"] = STATE["equity_history"][-300:]
        # Update drawdown from equity history
        try:
            eq_vals = [e.get("equity", 0) for e in STATE["equity_history"]]
            if eq_vals:
                peak = max(eq_vals)
                cur = eq_vals[-1]
                dd = 0.0 if peak <= 0 else max(0.0, (peak - cur) / peak)
                STATE["metrics"]["drawdown"] = round(dd, 4)
        except Exception:
            pass
    except Exception as e:
...
async def metrics():
    ...
    # Basic perf metrics
    try:
        durs = list(STATE["perf"]["loop_durations_ms"]) or []
        if durs:
            s = sorted(durs)
    ...
    recency = None
    try:
        if STATE["perf"]["trades_refresh_ts"]:
            recency = max(0.0, time.time() - STATE["perf"]["trades_refresh_ts"])
    except Exception:
        pass
    out = dict(STATE["metrics"])  # copy
    out.update({
        "loop_ms_p95": round(p95, 2),
        "data_recency_seconds": recency,
        "api_error_count": STATE["server_stats"].get("error_count", 0),
    })
    return out
...
@app.get("/feed/ohlcv")
async def feed_ohlcv(symbol: str = "BTC/USDT"):
    series = STATE["ohlcv"].get(symbol, [])
    return {"symbol": symbol, "ohlcv": series}
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        ...
        while True:
                ...
                json.dumps(
                    {
                        ...
                        "activity": STATE["activity"],
                        "equity_history": STATE["equity_history"],
                        "ohlcv": {"symbol": "BTC/USDT", "series": STATE["ohlcv"].get("BTC/USDT", [])},
                        "server_stats": {
                            "uptime_seconds": time.time() - STATE["server_stats"]["start_time"],
                            "active_connections": len(manager.active_connections),
                        },
            ...
            try:
                STATE["perf"]["loop_durations_ms"].append(dur_ms)
            except Exception:
                pass
...
def _init_ohlcv(symbol: str = "BTC/USDT", points: int = 120):
    base = 68000.0
    t = int(time.time()) - points * 3
    series = []
    last = base
    for i in range(points):
        o = last
        change = random.uniform(-0.003, 0.003)
        c = max(0.0, o * (1 + change))
        h = max(o, c) * (1 + random.uniform(0.0, 0.001))
        l = min(o, c) * (1 - random.uniform(0.0, 0.001))
        v = random.uniform(10, 100)
        series.append({"ts": (t + i * 3) * 1000, "o": round(o, 2), "h": round(h, 2), "l": round(l, 2), "c": round(c, 2), "v": round(v, 3)})
        last = c
    STATE["ohlcv"][symbol] = series[-300:]


async def _updater_task():
    while True:
        try:
            ...
            # advance OHLCV for default symbol
            sym = "BTC/USDT"
            series = STATE["ohlcv"].get(sym, [])
            last = series[-1]["c"] if series else 68000.0
            o = last
            change = random.uniform(-0.003, 0.003)
            c = max(0.0, o * (1 + change))
            h = max(o, c) * (1 + random.uniform(0.0, 0.001))
            l = min(o, c) * (1 - random.uniform(0.0, 0.001))
            v = random.uniform(10, 100)
            series.append({"ts": int(time.time()) * 1000, "o": round(o, 2), "h": round(h, 2), "l": round(l, 2), "c": round(c, 2), "v": round(v, 3)})
            STATE["ohlcv"][sym] = series[-300:]
            if random.random() < 0.4:
...
async def _trades_watcher_task():
    while True:
        try:
...
async def lifespan(app: FastAPI):
    _init_ohlcv("BTC/USDT", points=120)
    updater = asyncio.create_task(_updater_task())
    trades = asyncio.create_task(_trades_watcher_task())
    try:
        yield
    finally:
        for task in (updater, trades):
            task.cancel()
