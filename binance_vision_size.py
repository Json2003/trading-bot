...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("binance_crawl.log"), logging.StreamHandler()],
)
...
class ProgressTracker:
    ...
    def __init__(self, show_progress: bool = False):
        ...
        if show_progress and tqdm_available:
            self.progress_bar = tqdm(desc="Crawling", unit="req", dynamic_ncols=True, leave=True)
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            "completion_percentage": (self.completed_requests / self.total_requests * 100)
...
class AsyncBinanceCrawler:
    ...
    def __init__(
        self, concurrent_requests: int = 10, timeout: int = 30, show_progress: bool = False
    ):
        ...
        logger.info(
            f"Async crawler initialized: concurrent={concurrent_requests}, timeout={timeout}s"
        )
    ...
    async def __aenter__(self):
        ...
        timeout_config = aiohttp.ClientTimeout(
            total=self.timeout, connect=10, sock_read=self.timeout
        )
                    ...
                    async with self.session.get(url) as response:
                        if response.status == 200:
                            ...
                            if attempt < retries:
                                ...
                                logger.warning(
                                    f"Server error {response.status}, retrying in {wait_time:.1f}s"
                                )
    ...
    async def parse_s3_xml(
        self, content: str
    ) -> Optional[Tuple[List[Tuple[str, int]], List[str], bool, Optional[str]]]:
    ...
    async def parse_html(
    ...
    ) -> Optional[Tuple[List[Tuple[str, int]], List[str], Optional[str]]]:
        ...
        try:
            ...
            logger.warning(
                "HTML parsing is basic implementation - consider using BeautifulSoup for production"
            )
    ...
    async def crawl_prefix_async(self, prefix: str) -> Tuple[int, Dict[str, int]]:
        ...
        while queue:
            ...
            for cur_prefix, token in current_batch:
                ...
                # Build URL
                params = {"list-type": "2", "delimiter": "/", "prefix": cur_prefix}
                if token:
    ...
    async def process_single_listing(
        self, url: str, base_prefix: str, per_bucket: Dict[str, int]
    ) -> Optional[Tuple[List, List, bool, Optional[str], int]]:
...
def make_sync_session(timeout: int = 30):
    ...
    session.headers.update({"User-Agent": "Enhanced-Binance-Vision-Crawler/2.0-sync"})
...
def s3_list_xml(session, prefix, continuation_token=None):
    ...
    is_truncated = (
        root.find(f"{ns}IsTruncated") is not None and root.find(f"{ns}IsTruncated").text == "true"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute total size of Binance public data (data.binance.vision)"
    )
    ap.add_argument(
        "--prefix",
        default="data/",
        help="Prefix to crawl (default: data/). Examples: data/spot/, data/futures/um/, data/futures/cm/, data/options/",
    )
