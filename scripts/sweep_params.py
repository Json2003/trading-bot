...
def main():
    ...
    ap.add_argument(
        "--strategy_args", default="", help="Comma-separated key=value pairs to pass to strategy"
    )
    ...
    ap.add_argument(
        "--exit_mode", choices=["bps", "atr"], default="bps", help="Which exit type to sweep"
    )
    ap.add_argument(
        "--grid_atr_pct",
        default="0.30,0.35,0.40,0.45,0.50,0.55",
        help="Used only if strategy accepts atr_pctile",
    )
    ...
    if args.source == "csv":
        if not args.path:
            raise SystemExit("--path required for --source csv")
        df = load_csv(args.path)
    else:
        for k in ["exchange", "symbol", "timeframe", "since", "until"]:
            if not getattr(args, k):
                raise SystemExit(f"--{k} required for --source ccxt")
    ...
    if args.exit_mode == "bps":
        ...
        for i, (atr_pct, tp_bps, sl_bps, max_bars) in enumerate(combos, start=1):
            ...
            print(
                f"[{i}/{total}] mode=bps ATR%={(atr_pct if atr_pct is not None else float('nan')):.2f} "
        ...
        for i, (atr_pct, tp_atr, sl_atr, max_bars) in enumerate(combos, start=1):
            ...
            print(
                f"[{i}/{total}] mode=atr ATR%={(atr_pct if atr_pct is not None else float('nan')):.2f} "
