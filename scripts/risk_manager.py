...
def check_killswitch():
    ...
    ceq = 0.0
    cpath = os.path.join(ART, "crypto_heartbeat.json")
    if os.path.exists(cpath):
        with open(cpath) as f:
            h = json.load(f)
        for row in h.get("equities", []):
            ceq += row.get("equity", 1.0)
        if h.get("equities"):
            ceq /= max(len(h["equities"]), 1)
    eeq = 0.0
    epath = os.path.join(ART, "equity_rotation_financials_metrics.json")
    if os.path.exists(epath):
        with open(epath) as f:
            m = json.load(f)
            eeq = 1.0 + float(m.get("total_return", 0.0))
    ...
    if len(state["equity_hist"]) > 1440:
        state["equity_hist"] = state["equity_hist"][-1440:]

    # daily change
    today = [x for x in state["equity_hist"] if x["ts"][:10] == ts[:10]]
    if today:
        open_eq = today[0]["eq"]
        daily_ret = (eq / open_eq) - 1.0
        if daily_ret <= DAILY and not state["halted"]:
            state["halted"] = True
            notify(f"DAILY LOSS HALT {daily_ret:.2%}. Halting bots.")
    ...
    if dd <= RUN_DD and not state["halted"]:
        state["halted"] = True
        notify(f"RUNNING DD HALT {dd:.2%}. Halting bots.")
    ...
    if h:
        ...
        if state["loss_streak"] >= MAX_LOSS_STREAK and not state["halted"]:
            state["halted"] = True
            notify(f"LOSS STREAK HALT n={state['loss_streak']}. Halting bots.")
