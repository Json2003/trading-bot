#!/usr/bin/env python3
"""Evaluate one frozen cross-asset boom/bust regime hypothesis."""
...
def evaluate(
    ...
    for current in signal_dates:
        regime = regime_at(macro, emas, current)
        if regime is None or regime[0] == 0:
            continue
        if last_signal is not None and (current - last_signal).days < COOLDOWN_DAYS:
            continue
        row = _trade(bars[asset], current, regime[0], asset)
        if row is None or date.fromisoformat(row["exit_date"]) >= end:
            continue
        row.update({"regime": "boom" if regime[0] > 0 else "bust", "risk_on_score": regime[1], "risk_off_score": regime[2]})
        rows.append(row)
        last_signal = current
...
def main() -> int:
    ...
    report = {
        ...
        "hypothesis": "A confirmed cross-asset boom or bust regime predicts same-direction BTC/ETH continuation over the next five trading days.",
        "frozen_parameters": {
            ...
            "entry": "first BTC/ETH daily bar after the completed macro session plus one latency bar",
            "notional": NOTIONAL,
        },
